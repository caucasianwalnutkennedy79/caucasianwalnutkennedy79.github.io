#!/usr/bin/env python3
"""Run the boat-banner regression harness in headless Firefox."""
...
PROTOTYPES_DIR = TESTS_DIR.parent
EXPECTED_CASES = 838
...
def print_results(browser_version, results, issues):
    ...
    print("\nExtra checks")
    for section_name in ("telemetryChecks", "pointerChecks", "shipChecks"):
...
def run(args):
    ...
    client = None
    try:
        handler = functools.partial(QuietHandler, directory=str(PROTOTYPES_DIR))
        server = http.server.ThreadingHTTPServer(("127.0.0.1", 0), handler)
        server_thread = threading.Thread(target=server.serve_forever, daemon=True)
        server_thread.start()
        http_port = server.server_address[1]
        harness_path = args.harness.lstrip("/")
        harness_url = f"http://127.0.0.1:{http_port}/{harness_path}"
        try:
